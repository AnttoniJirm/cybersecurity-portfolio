...
- [ ] To review: DNS poisoning techniques in depth — how attackers execute it and how to detect it
- [ ] To review: TLS handshake process — how HTTPS actually establishes an encrypted session

---

## Portfolio connections
- **VirtualBox Home Lab** — static IP configuration, internal network isolation, and ping tests all applied the networking concepts covered here directly
- **Linux Fundamentals**  `ifconfig`, `ssh`, `ip a` commands connect directly to IP addressing and remote access protocols
- **Windows Fundamentals**  `ipconfig`, `netstat` are the Windows equivalents of the Linux networking commands
- **Next report — Network Tools**  Wireshark, Nmap, Tcpdump, and Metasploit all operate at specific OSI layers covered in this report
- **AZ-900**  Azure Virtual Networks, NSGs (Network Security Groups), and VPN Gateway all build on these networking fundamentals
